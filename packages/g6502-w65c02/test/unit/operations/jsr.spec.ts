    ...
    describe('6502', () => {
        describe('jsr', () => {
            it('should push the program counter to the stack and set program counter equal to parameter', () => {
                ...
                expect(bus.write.mock.calls.length).toBe(2)

                expect(bus.write.mock.calls[0]![0]).toEqual(0x0178)
                expect(bus.write.mock.calls[0]![1]).toEqual(0x12)

                expect(bus.write.mock.calls[1]![0]).toEqual(0x0177)
                expect(bus.write.mock.calls[1]![1]).toEqual(0x34 - 1)
